...
namespace Berryfy.Infrastructure.Data
{
    public class DataSeeder
    {
        ...
        public async Task SeedDataAsync()
        {
            ...
            var runSeedOnStartup = Environment.GetEnvironmentVariable("Database__RunSeedOnStartup");
            if (!string.Equals(env, "Development", StringComparison.OrdinalIgnoreCase) &&
                !string.Equals(runSeedOnStartup, "true", StringComparison.OrdinalIgnoreCase))
            {
                Console.WriteLine("Seeder skipped: set Database__RunSeedOnStartup=true to seed in non-Development environments.");
                return;
            }
        ...
        private async Task SeedShopAsync()
        {
            ...
            if (!await _context.Shops.AnyAsync())
            {
                var shop = new Shop
                {
                    ...
                    Email = "info@Berryfy.com",
                    Phone = "+111111111111",
                    Address = "Dulkadiroglu",
                    Currency = "TR",
                    Language = "Türkçe",
                    LogoUrl = "Berryfy Sample Logo"
                };
...
new Product
{
    ...
    ReservedStock = 0,
                    ...
                    // products[0..3]=VF, [4..7]=WD, [8..11]=SN, [12..15]=BF
                    var productCategories = products
                        .Select((p, i) => new ProductCategory
                        {
                            ProductId = p.Id,
                            CategoryId = i < 4 ? vegetablesFruits!.Id
                                       : i < 8 ? waterDrinks!.Id
                                       : i < 12 ? snack!.Id
                                       : breakfast!.Id,
                            CreatedAt = DateTime.UtcNow,
